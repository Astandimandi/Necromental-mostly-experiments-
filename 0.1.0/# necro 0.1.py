...
# knife crafting doesn't update, equip either
# same with rope
# bone knife doesn't activate in any way at all lol 
# same with crates
...
have_bone_knife = tk.BooleanVar(value=False)
have_crates = tk.BooleanVar(value=False)
have_skeleton = tk.BooleanVar(value=False)

# Crafting requirements
pay_for_item = {
    "shovel": {"sticks": 10, "stones": 5},
    "knife": {"sticks": 3, "stones": 1, "rope": 1},
    "rope": {"sticks": 6},
    "bucket": {"sticks": 10, "rope": 2},
    "crates": {"sticks": 20, "rope": 4},
    "worktable": {"sticks": 30, "rope": 5, "stones": 5},
    "bone knife": {"bone shards": 2, "stones": 1, "rope": 1},
    "bone": {"bone shards": 5},
    "skeleton": {"bones": 5, "buckets": 1, "dirt": 10},
}
...
# Info boxes - setup and updates, main interface of the game
def update_news():
    ...
    if shovel_up.get() > 0:
        messages.append("You're using the shovel!")
    if bucket_up.get() > 0:
...
def update_equipment_board():
    ...
    if have_shovel.get():
        equipment_box.insert(tk.END, "Shovel\n")
    if have_bucket.get():
        equipment_box.insert(tk.END, "Bucket\n")
    if have_knife.get():
        equipment_box.insert(tk.END, "Knife\n")
    if have_bone_knife.get():
        equipment_box.insert(tk.END, "Bone Knife\n")
    if have_crates.get():
        equipment_box.insert(tk.END, "Crates\n")
    if have_skeleton.get():
        equipment_box.insert(tk.END, "Skeleton\n")
...
def crafting(item_name):
    ...
    if item_name == "shovel":
        if (stick_up.get() >= raw_materials.get("sticks", 0) and 
            stone_up.get() >= raw_materials.get("stones", 0)):
    ...
    if item_name == "bone":
        if bone_shard_up.get() >= raw_materials.get("bone shards", 0):
            bone_shard_up.set(bone_shard_up.get() - raw_materials.get("bone shards", 0))
    ...
    if item_name == "knife":
        if (stick_up.get() >= raw_materials.get("sticks", 0) and 
            stone_up.get() >= raw_materials.get("stones", 0) and 
            rope_up.get() >= raw_materials.get("rope", 0)):
            stick_up.set(stick_up.get() - raw_materials.get("sticks", 0))
            stone_up.set(stone_up.get() - raw_materials.get("stones", 0))
    ...
    if item_name == "rope":
        if stick_up.get() >= raw_materials.get("sticks", 0):
            stick_up.set(stick_up.get() - raw_materials.get("sticks", 0))
            rope_up.set(rope_up.get() + 1)
            update_equipment_board()
            update_materials()
            update_news()

    if item_name == "bone knife":
        if (bone_shard_up.get() >= raw_materials.get("bone shards", 0) and 
            stone_up.get() >= raw_materials.get("stones", 0) and 
            rope_up.get() >= raw_materials.get("rope", 0)):
            bone_shard_up.set(bone_shard_up.get() - raw_materials.get("bone shards", 0))
            stone_up.set(stone_up.get() - raw_materials.get("stones", 0))
            rope_up.set(rope_up.get() - raw_materials.get("rope", 0))
            bone_knife_up.set(bone_knife_up.get() + 1)
            update_equipment_board()
            update_materials()
            update_news()
